...
Examples:
- On a successful recall, emit a short actionable summary + a concrete next step.
...
- **Domain label (SSOT)**: Post-Mission Analysis / Data Repository (Assimilator) / AAR / learning / assimilation
- **What the port produces**: Indexed memory, exemplars, retrieval surfaces, SSOT snapshots
- **What the port rejects**: Unprovenanced memory writes, orphaned artifacts, duplicate truths

Galois handoff (P1 + P6 stage 1):
- **P1 → P6**: schemas + manifests that make ingestion safe and queryable.
...
- Decay/curation flow (remove/retire low-signal items)
...
- Information flows: retrieval surfaces that prevent agents from hallucinating (Homing)
