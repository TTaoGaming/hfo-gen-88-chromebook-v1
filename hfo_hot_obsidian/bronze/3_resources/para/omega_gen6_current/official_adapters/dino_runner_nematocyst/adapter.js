...
(function () {
  ...
  const inject = (payload, frameEl) => {
    ...
    if (action !== 'keypress') return { ok: false, reason: 'unsupported_action' };

    const ok = dispatchKeypress(frameEl, {
      key: clampString(payload.key, ' '),
      code: clampString(payload.code, 'Space')
    });
