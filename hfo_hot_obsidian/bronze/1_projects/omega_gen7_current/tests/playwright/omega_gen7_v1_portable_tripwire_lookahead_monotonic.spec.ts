...
import { safeGoto, safeEvaluate, buildLightUrl, GEN7_V1_PORTABLE_V23_10_URL_BASE } from './_support';
...
const FIXTURE_PATH =
  'hfo_hot_obsidian/bronze/1_projects/omega_gen7_v1_portable/tests/fixtures/touch2d/gen6_v17_tripwire_lookahead_slow_cross_golden.jsonl';

test('Gen7 portable v23.10: raising lookahead window increases inject lead time', async ({ page }) => {
  const frames = fs
  ...
  const measureLead = async (lookaheadWindowMs: number) => {
    const url = buildLightUrl(
      ...
      `flag-p3-tripwire-injector=true&flag-p3-tripwire-injector-static=true&test-run=${encodeURIComponent(String(lookaheadWindowMs))}`,
    );
  ...
  // v23.10 emits a `sensorId` field on `tripwire_cross` (e.g. "static"), which is richer than older strict schemas.
  // For this portable regression check, validate minimally but fail-closed on missing planck engine + timestamps.
