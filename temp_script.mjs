        ...
        class JuiceSubstrate {
            ...
            update(cursors) {
                // Future WebGL Fire/Juice implementation
            }
                    ...
                    if (landmarks) {
                        try {
                            ...
                            if (systemState.parameters.physics.mode === 'STATIC_ROD') {
                                ...
                                // Predictive Lookahead (Optional, based on velocity)
                                const telemetry = systemState.p1.physicsState[i].getTelemetry();
                                finalX = physicsPos.x + telemetry.velocity.x * (systemState.parameters.physics.predictiveLookahead / 100);
                                finalY = physicsPos.y + telemetry.velocity.y * (systemState.parameters.physics.predictiveLookahead / 100);
                            }
                            ...
                            // 💧 Temporal Leaky Bucket Logic (V16: Coastal Drain Aware)
                            if (isPalmFacing) {
            ...
            if (systemState.ui.gestures) {
                ...
                if (results.landmarks && results.landmarks.length > 0) {
                    results.landmarks.forEach((_, i) => {
                        ...
                        const labelPrefix = isPrimary ? '🎖️ [PRIMARY] ' : '';
            ...
            if (systemState.ui.palmCtx) {
                ...
                if (systemState.p1.cursors.length > 0) {
                    systemState.p1.cursors.forEach((c, i) => {
                        ...
                        // 🟢 Palm Orientation Sphere (Enhanced V14)
        ...
        function drawResults(results) {
            ...
            cursors.forEach((c, i) => {
                ...
                if (systemState.parameters.physics.showLaserBeam) {
                    ...
                    ctx.strokeStyle = color;
                    ctx.lineWidth = fsmState === 'COMMIT' ? 4 : 2;
                    ctx.globalAlpha = fsmState === 'COMMIT' ? 0.8 : 0.4;
                    ctx.stroke();
                    ctx.globalAlpha = 1.0;
                }
                ...
                // Tip/Dot
                ctx.beginPath();
                ctx.arc(screenX, screenY, fsmState === 'COMMIT' ? 6 : 4, 0, Math.PI * 2);
                ctx.fillStyle = color;
                ctx.fill();
