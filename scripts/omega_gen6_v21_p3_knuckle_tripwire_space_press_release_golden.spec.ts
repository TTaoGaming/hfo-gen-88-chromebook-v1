...
// v21 BDD/TDD: confirm that the golden master COMMIT knuckle tripwire begin/end edges inject Space keydown/keyup.
...
test('Gen6 v21: golden master COMMIT knuckle tripwire → Space keydown/keyup', async ({ hfoPage }) => {
    const lines = fs
    ...
    // Golden master may include extra later crossings; require at least one clean press+release.
    ...
    expect(actions.slice(0, 2), `Expected first pair keydown+keyup; got=${JSON.stringify(injects)}`).toEqual(['keydown', 'keyup']);
});
