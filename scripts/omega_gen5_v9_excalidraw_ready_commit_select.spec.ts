...
const GEN5_V9_URL =
    ...
    'http://localhost:8889/hfo_hot_obsidian/bronze/1_projects/omega_gen5_current/omega_gen5_v9.html'
...
test('Gen5 v9: READY hover + COMMIT press selects Excalidraw tool on COMMIT (not release)', async ({ hfoPage }) => {
    await hfoPage.goto(GEN5_V9_URL);
    ...
    await hfoPage.evaluate(() => {
        ...
        // @ts-ignore
        window.__hfoV9TapLog = { over: 0, down: 0, up: 0, click: 0, errors: [] as string[] };
    });
    ...
    await frame.locator('body').evaluate(() => {
        // @ts-ignore
        const log = window.parent.__hfoV9TapLog;
