...
const GEN5_URL =
    ...
    'http://localhost:8889/hfo_hot_obsidian/bronze/1_projects/omega_gen5_current/omega_gen5_v4_1.html?flag-engine-babylon=false&flag-engine-canvas=true&flag-disable-camera=true&flag-ui-excalidraw=true&flag-p3-legacy-click=false';

test('RED: pointer emulation should generate click on pointerup', async ({ hfoPage }) => {
    ...
    await hfoPage.evaluate(() => {
        ...
        // @ts-ignore
        window.__hfoEmuLog = { click: 0, down: 0, up: 0, errors: [] };
    });
    ...
    await frame.locator('body').evaluate(() => {
        // @ts-ignore
        const log = window.parent.__hfoEmuLog;
    ...
    const results = await hfoPage.evaluate(() => {
        // @ts-ignore
        return window.__hfoEmuLog;
    });

    expect(results.errors).toEqual([]);
    expect(results.down).toBeGreaterThan(0);
    expect(results.up).toBeGreaterThan(0);
    // RED expectation: click should be synthesized as part of mouse emulation on pointerup
    expect(results.click).toBeGreaterThan(0);
});
