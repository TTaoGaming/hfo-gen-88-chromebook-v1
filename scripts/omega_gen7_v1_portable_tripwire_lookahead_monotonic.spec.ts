...
const FIXTURE_PATH =
  "hfo_hot_obsidian_forge/1_silver/0_projects/omega_gen7_unified/tests/fixtures/touch2d/gen6_v17_tripwire_lookahead_slow_cross_golden.jsonl";

const GEN7_V1_URL_BASE =
  "http://localhost:8889/hfo_hot_obsidian_forge/1_silver/0_projects/omega_gen7_unified/app/omega_gen7_v1.html";

test("Gen7 v1: raising lookahead window increases inject lead time", async ({
  hfoPage,
}) => {
  const frames = fs
  ...
  const measureLead = async (lookaheadWindowMs: number) => {
    ...
    return safeEvaluate(
      ...
      (args) => {
        ...
        if (!w.hfoPortsEffects?.getRecent)
          throw new Error("missing hfoPortsEffects.getRecent");
        if (!w.hfoPortsEffects?.clear)
          throw new Error("missing hfoPortsEffects.clear");
        if (!w.hfoP2TripwireThread?.tick)
          throw new Error("missing hfoP2TripwireThread.tick");
        if (!w.hfoP3PlanckSensorInjector?.tick)
          throw new Error("missing hfoP3PlanckSensorInjector.tick");
        if (!w.hfoState?.parameters)
          throw new Error("missing hfoState.parameters");

        // Port7-backed knobs: GUI is a view onto these.
        w.hfoState.parameters.p2 = w.hfoState.parameters.p2 || {};
        w.hfoState.parameters.p2.tripwireThread =
          w.hfoState.parameters.p2.tripwireThread || {};
        w.hfoState.parameters.p2.tripwireThread.lookaheadEnabled = true;
        w.hfoState.parameters.p2.tripwireThread.lookaheadWindowMs =
          args.lookaheadWindowMs;

        w.hfoState.parameters.p3 = w.hfoState.parameters.p3 || {};
        w.hfoState.parameters.p3.tripwireInjector =
          w.hfoState.parameters.p3.tripwireInjector || {};
        ...
        const relevant = recent
          .filter(
            (e: any) =>
              (e?.port === "p2" || e?.port === "p3") &&
              (e?.type === "tripwire_lookahead" ||
                e?.type === "tripwire_cross" ||
                e?.type === "tripwire_inject"),
          )
          .map((e: any) => ({
            port: e.port,
            type: e.type,
        ...
        const p2Look = relevant.filter(
          (e: any) => e.port === "p2" && e.type === "tripwire_lookahead",
        );
        const p2Cross = relevant.filter(
          (e: any) => e.port === "p2" && e.type === "tripwire_cross",
        );
        const p3Inject = relevant.filter(
          (e: any) => e.port === "p3" && e.type === "tripwire_inject",
        );
        ...
        const leadMs =
          Number.isFinite(crossNow) && Number.isFinite(injectNow)
            ? crossNow - injectNow
            : NaN;
        ...
        return {
          ...
          leadMs,
          counts: {
            lookahead: p2Look.length,
            cross: p2Cross.length,
            inject: p3Inject.length,
          },
          samples: {
  ...
  if (low.samples.firstLookahead)
    TripwireLookaheadV1Schema.parse(low.samples.firstLookahead);
  if (high.samples.firstLookahead)
    TripwireLookaheadV1Schema.parse(high.samples.firstLookahead);
  ...
  expect(
    low.counts.cross,
    "expected >=1 p2/tripwire_cross @ window=0ms",
  ).toBeGreaterThanOrEqual(1);
  expect(
    low.counts.inject,
    "expected >=1 p3/tripwire_inject @ window=0ms",
  ).toBeGreaterThanOrEqual(1);
  expect(low.counts.lookahead, "expected 0 lookahead @ window=0ms").toBe(0);

  expect(
    high.counts.cross,
    "expected >=1 p2/tripwire_cross @ window=200ms",
  ).toBeGreaterThanOrEqual(1);
  expect(
    high.counts.inject,
    "expected >=1 p3/tripwire_inject @ window=200ms",
  ).toBeGreaterThanOrEqual(1);
  expect(
    high.counts.lookahead,
    "expected >=1 lookahead @ window=200ms",
  ).toBeGreaterThanOrEqual(1);
