...
// Visual sanity check (not a pixel-golden): render v23.9 with synthetic sword marker and capture a screenshot.
// This is meant to be deterministic enough for human review without relying on WebGL pixel goldens.

test.describe.configure({ mode: 'serial', retries: 1 });

test('Gen6 v23.9: pyreblade flamberge amber core screenshot', async ({ hfoPage }) => {
    const url =
    ...
    await hfoPage.evaluate(() => {
        ...
        try {
            ...
            if (jl0?.canvas) jl0.canvas.style.display = 'block';
        } catch {
            // ignore
        }
    ...
    await hfoPage.waitForTimeout(400);

    const outDir = path.join(process.cwd(), 'test-results');
    fs.mkdirSync(outDir, { recursive: true });
    const outPath = path.join(outDir, 'omega_gen6_v23_9_pyreblade_flamberge_amber.png');

    await hfoPage.screenshot({ path: outPath, fullPage: true });
