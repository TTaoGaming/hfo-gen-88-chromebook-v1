...
test('V42: FSM Intent Recognition (Direct FSM Test)', async ({ hfoPage }) => {
...
test('V42: Release Logic (Direct FSM Test)', async ({ hfoPage }) => {
  const url = getActiveUrl();
  await hfoPage.goto(url);
  await hfoPage.initHFO();
  await hfoPage.waitForHand(0);
...
test('V42: Stickiness through NONE and Frame Drops', async ({ hfoPage }) => {
  const url = getActiveUrl();
  await hfoPage.goto(url);
  await hfoPage.initHFO();
  await hfoPage.waitForHand(0);
...
test('V42: Return to READY on Any Other Gesture', async ({ hfoPage }) => {
  const url = getActiveUrl();
  await hfoPage.goto(url);
  await hfoPage.initHFO();
  await hfoPage.waitForHand(0);
