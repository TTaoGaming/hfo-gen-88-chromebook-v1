...
# Ensure gravity and emitRate are optimized
content = re.sub(
    r'fire\.minLifeTime = 0\.05; fire\.maxLifeTime = 0\.15;.*?fire\.emitRate = 400;.*?fire\.gravity = new BABYLON\.Vector3\(0, 15, 0\);',
...
new_visuals = """// State based visuals (V24.10: PHX CORE with COAST Fading)
       if (c.fsmState === 'COMMIT' || c.fsmState === 'COAST') {
           ...
           systems.forEach(s => {
               ...
               if (s.name === "fire") {
                   ...
                   s.color1 = new BABYLON.Color4(1, 0.2, 0, opacity);
               }
           });
       } else {
           systems.forEach(s => {
               ...
               if (s.name === "fire") {
                   s.emitRate = 800;
                   s.color1 = new BABYLON.Color4(1, 0.7, 0.1, 1.0);
               }
           });
       }"""

# Target the V24.7 block
content = re.sub(
