...
This module provides a tiny helper used by scripts to store a small, structured
status update into the SSOT without requiring an MCP server.
"""
...
def resolve_ssot_sqlite_path(*, repo_root: Path | None = None) -> str:
    ...
    # Always validate the resolved target against pointer-blessed SSOT.
...
def main(argv: list[str]) -> int:
    ...
    ok, msg = asyncio.run(
        store_status_update(
            ...
            metadata={"source": "hfo_ssot_status_update.py"},
        )
    )
