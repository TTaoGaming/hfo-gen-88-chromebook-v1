...
- **V84.5 Hot Bronze (2026-01-16)**: **OMEGA V40.0 INITIALIZED**. Evolved V39.5 to V40.0. Resolved the "Stuck Idle Cursor" bug where the yellow charge dot remained visible during tracking loss. Synchronized `idleIndicator` alpha with `skeletonAlpha` and enforced recursive group disabling if `fadeAlpha < 0.01`. Verified 1:1 parity and visual lifecycle stability via P5 Forensic Audit. [Receipt: SUCCESS_V40_0_IDLE_FIX]
