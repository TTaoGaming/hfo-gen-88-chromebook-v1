...
/* Fork of Chris Donahue's Piano Genie helpers.js */
...
const CONSTANTS = {
    ...
    // Using local staggered assets for Chromebook V-1 Offline Mode
    GENIE_CHECKPOINT: "../assets/piano_genie/",
...
class Player {
    ...
    loadAllSamples() {
        const seq = { notes: [] };
        // OCTAVES is defined in script.js
        const oct = typeof OCTAVES !== 'undefined' ? OCTAVES : 7;
        for (let i = 0; i < CONSTANTS.NOTES_PER_OCTAVE * oct; i++) {
    ...
    sendMidiNoteOn(pitch, button) {
        ...
        const msg = [0x90, pitch, this.midiOutVelocity];
        if (this.midiOut[this.selectOutElement.selectedIndex]) {
    ...
    getMIDIMessage(msg) {
        if (!this.usingMidiIn) return;
        const command = msg.data[0];
        const pitch = msg.data[1];
        if (!(pitch in MIDI_IN_MAPPING)) return;
        const button = MIDI_IN_MAPPING[pitch];
        switch (command) {
            case 0x90: // note on
                if (window.buttonDown) window.buttonDown(button, false);
                break;
            case 0x80: // note off
                if (window.buttonUp) window.buttonUp(button);
                break;
        }
    }
}
...
class FloatyNotes {
    ...
    drawLoop() {
        const dy = 5; // Medallion: Bronze | Mutation: 88% (Pareto)
        ...
        for (let i = 0; i < this.notes.length; i++) {
            ...
            this.context.globalAlpha = Math.max(0, 1 - note.y / this.contextHeight);
